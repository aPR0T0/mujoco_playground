...
# ==============================================================================
"""Joystick Phase task for Open Duck Mini V2. (based on Berkeley Humanoid Phase concept)"""
...
from mujoco_playground._src.locomotion.miniv2 import miniv2_constants as constants
from mujoco_playground._src.locomotion.miniv2 import base as open_duck_mini_v2_base
...
class Joystick(open_duck_mini_v2_base.OpenDuckMiniV2Env):
    """Track a joystick command with phase-based gait."""

    def __init__(
        self,
        task: str = "no_head_flat_terrain",
    ...
    def _post_init(self) -> None:
        ...
        # Get hip and knee indices (adapting from Berkeley Humanoid)
        hip_indices = []
        for name in constants.JOINTS_ORDER_NO_HEAD:
            if "_hip" in name:
                idx = constants.JOINTS_ORDER_NO_HEAD.index(name)
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        ...
        rng, key = jax.random.split(rng)
        qpos = qpos.at[7:].set(
            qpos[7:] * jax.random.uniform(key, (self.mjx_model.nu,), minval=0.9, maxval=1.1)
        )

        # d(xyzrpy)=U(-0.5, 0.5)
        rng, key = jax.random.split(rng)
        qvel = qvel.at[0:6].set(
            jax.random.uniform(key, (6,), minval=-0.03, maxval=0.03)
        )
        ...
        rng, key = jax.random.split(rng)
        gait_freq = jax.random.uniform(key, (1,), minval=0.5, maxval=1.0)
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        motor_targets = self._default_actuator + action * self._config.action_scale
        data = mjx_env.step(
    ...
    def _cost_stand_still(
        ...
        return jp.sum(jp.abs(qpos - self._default_actuator)) * (cmd_norm < 0.1)
    ...
    def _cost_joint_deviation_hip(
        ...
        cost = jp.sum(
            jp.abs(qpos[self._hip_indices] - self._default_actuator[self._hip_indices])
        )
    ...
    def _cost_joint_deviation_knee(self, qpos: jax.Array) -> jax.Array:
        return jp.sum(
            jp.abs(
                qpos[self._knee_indices] - self._default_actuator[self._knee_indices]
            )
        )

    def _cost_pose(self, qpos: jax.Array) -> jax.Array:
        return jp.sum(jp.square(qpos - self._default_actuator) * self._weights)
